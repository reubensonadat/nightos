...
export function BrandSettingsScreen({ venueId }: Props) {
  ...
  let previewTotal = previewBase;

  if (vatEnabled && vatRate > 0) {
    if (taxInclusive) {
      // Menu price includes VAT: Total is GH₵ 100.00, VAT is extracted, net subtotal + VAT = 100.00
      ...
      // Tax Exclusive: VAT added at checkout
      ...
      <form onSubmit={handleSave} className="space-y-8">
        ...
        <div className="rounded-[1.75rem] border border-licorice/8 bg-white p-6 sm:p-8 shadow-[0_4px_20px_rgba(35,20,12,0.03)] space-y-6">
          <div className="flex items-center gap-3 border-b border-isabelline pb-4">
            ...
            <div>
              <h2 className="font-serif text-lg font-bold text-licorice">Taxes & Pricing Rules</h2>
              <p className="text-xs text-feldgrau">
                Configuration for Value Added Tax (VAT), menu pricing display, and guest payment workflow.
              </p>
            </div>
          </div>
