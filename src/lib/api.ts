...
export const db = {
  ...
  /* ── Waiter Dashboard Linkages ── */
  getVenueSetting: async (venueId: string, key: string, fallback: any) => {
    const { data, error } = await supabase
      .from('venue_settings')
      .select('value')
      .eq('venue_id', venueId)
      .eq('key', key)
      .maybeSingle();
    if (error || !data) return { data: fallback, error };
    return { data: data.value ?? fallback, error: null };
  },

  staffNamesByIds: async (ids: string[]) => {
    if (!ids || ids.length === 0) return { data: [], error: null };
    return supabase.from('staff').select('id, name').in('id', ids);
  },
  ...
  transferBill: async (billId: string, destTableId: string, staffId: string) => {
    return { data: null, error: new Error('Transfer bill is a future feature') };
  },

  mergeBills: async (sourceBillId: string, destBillId: string, staffId: string) => {
    return { data: null, error: new Error('Merge bills is a future feature') };
  },

  splitBill: async (billId: string, ways: number, staffId: string) => {
    return { data: null, error: new Error('Split bill is a future feature') };
  },

  createOrderSubmission: async (billId: string, venueId: string, station: string, notes?: string) => {
    return supabase
      .from('order_submissions')
      .insert({
        bill_id: billId,
  ...
  createOrderItem: async (submissionId: string, billId: string, productId: string, productName: string, quantity: number, price: number, modifiers: any[], modifierTotal: number, lineTotal: number, notes?: string) => {
    return supabase
      .from('order_items')
      .insert({
        submission_id: submissionId,
  ...
  recordCashPayment: async (billId: string, amount: number, staffId: string) => {
