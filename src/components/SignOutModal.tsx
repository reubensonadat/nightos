        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
            ...
            <div className="relative z-10 w-full max-w-sm mx-4 md:mx-auto rounded-t-[28px] md:rounded-[1.5rem] bg-isabelline shadow-[0_-16px_48px_rgba(35,20,12,0.22)] ring-1 ring-white/60 overflow-hidden">
                ...
                <div className="px-6 pt-5 pb-2 md:pt-6">
                    <h2 className="text-[17px] font-bold tracking-tight text-licorice">Ready to clock out?</h2>
                    <p className="mt-2 text-[13px] leading-relaxed text-feldgrau">
                        You managed{" "}
                        <span className="font-bold text-licorice">{tablesManaged} table{tablesManaged !== 1 && "s"}</span>{" "}
                        today. Review your shift before signing out?
                    </p>
                </div>

                <div className="flex flex-col gap-2 px-6 pt-4 pb-[max(env(safe-area-inset-bottom),24px)]">
                    {/* Primary — review performance */}
                    <button
                        type="button"
                    ...
                    <button
                        type="button"
                        onClick={onClose}
                        className="w-full rounded-full bg-white py-3.5 text-[13px] font-bold tracking-tight text-feldgrau ring-1 ring-licorice/8 transition-colors hover:text-licorice active:scale-[0.98]"
                    >
                        Cancel
                    </button>
                </div>
